...
ages = np.arange(600.0, 900.1, 50.0)*1.0e6
masses = []
...
for age in ages:
    ...
    for f in files:
        try:
            ...
            trk = np.genfromtxt(f, skip_header=10, usecols=(2, 6, 5, 3, 4, 56, 8))
    ...
    all_props = icurve(np.arange(min(masses), 1.71, 0.01))
    ...
    np.savetxt('../iso/dmestar_{:07.1f}myr_z+0.10_a+0.00_phx.iso'.format(age/1.e6), all_props, fmt='%14.7f', header=header)
    del all_props
